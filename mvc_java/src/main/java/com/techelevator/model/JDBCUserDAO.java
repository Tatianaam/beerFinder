...
public class JDBCUserDAO implements UserDAO {
	...
	public void saveUser(User user) {
		...
		jdbcTemplate.update("INSERT INTO app_user(user_name, password, salt, email, phone, city, state, zip, favbeer, role, personal_name, birthday) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				user.getUserName(),
				hashedPassword, saltString, user.getEmail(), user.getPhone(), 
				user.getCity(), user.getState(), user.getZip(), user.getFavBeer(),
				user.getRole(), user.getPersonalName(),  
				user.getBirthday());
	}
	...
	public User getUserByUserName(String userName) {
		...
		if(user.next()) {
			...
			thisUser.setPersonalName(user.getString("personal_name"));
			thisUser.setPassword(user.getString("password"));
//			thisUser.setBirthday(user.getDate("birthday").toLocalDate());
			thisUser.setEmail(user.getString("email"));
			thisUser.setPhone(user.getString("phone"));
			thisUser.setCity(user.getString("city"));
			thisUser.setState(user.getString("state"));
			thisUser.setZip(user.getString("zip"));
			thisUser.setFavBeer(user.getString("favbeer"));
		}
